...
L.IM_ControlFons = L.Control.extend({
	...
	_addSidebarItem: function(container, elem)
	{

		var self = this;
		var item = L.DomUtil.create('div', 'fonsIcon');

		var itemIcon = L.DomUtil.create('div', elem.attr('class') + ' btFons');
		itemIcon.id = elem.attr('id');
		itemIcon.style = "margin-left: 2px;";
		var itemName = L.DomUtil.create('div', 'fonsName');
		itemName.innerHTML = window.lang.translate(elem.data('original-title'));

		item.appendChild(itemIcon);
		item.appendChild(itemName);

		L.DomEvent.on(item, 'click', L.DomEvent.stopPropagation);
		L.DomEvent.on(item, 'click', self._onItemClick, self);
	...
	moveToSidebar: function(sidebarId)
	{
		...
		var self = this;
		var buttonHTML = '<ul role="tablist"><li style="height: 50px;">' + 
			'<a href="#fonsPanel" id="fonsTab" role="tab"><div class="btn-sm bt_orto"><small id="sm_tipus" lang="ca">Fons</small></div></a></li></ul>';
		var bodyHTML = '<div id="fonsPanel" class="sidebar-pane sidebar-outer-pane">' +
			'<h1 class="sidebar-header" lang="ca">' +
				self.options.title + 
				'<span class="sidebar-close" id="infoClose"><i class="fa fa-caret-left"></i></span>' +
			'</h1><div class="scrollable-pane"><div id="fons-content"></div></div></div>';
		var btnsFons = $('.llista-fons div');
		$(sidebarId + ' .sidebar-tabs').append(buttonHTML);
		$(sidebarId + ' .sidebar-content').append(bodyHTML);

		for(var i=0, len=btnsFons.length; i<len; ++i)
		{
