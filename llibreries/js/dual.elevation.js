L.Control.Elevation = L.Control.extend({
	...
	this.options.scope='Cesium';
	 var opts = this.options;
		...
		$("#elevation_profile").append(container);
        return container;
    ...
    _mousemoveHandler: function(d, i, ctx) {
		...
		var _enti=viewer.entities.getById('tmp_point');			
		if(	_enti) {			
	...
	*/	   
	...
	_addDataCesium:function(coords){
        ...
        if (coords) {
            ...
            var ele = this._maxElevation || 0;
            for (var i = 0; i < coords.length; i++) {
                var s = new L.LatLng((coords[i].latitude *(180.0 / Math.PI)), (coords[i].longitude*(180.0 / Math.PI)));
                var e = new L.LatLng(coords[i ? i - 1 : 0].latitude *(180.0 / Math.PI), coords[i ? i - 1 : 0].longitude *(180.0 / Math.PI));
                var newdist = opts.imperial ? s.distanceTo(e) * this.__mileFactor : s.distanceTo(e);
                dist = dist + Math.round(newdist / 1000 * 100000) / 100000;
	...
	addDataCesium: function(d) {
		...
		try{
				this._addDataCesium(d);
				if (this._container) {
					this._applyData();
				}
				$("#elevation_profile").show();
